...
| **Transformation** | [dbt Core](https://www.getdbt.com/) | Modern SQL-based ELT modeling and data transformations | ⏳ Upcoming |
...
### 2. The Python Ingestion Producer
A lightweight, containerized Python script. Upon establishing the socket connection, it negotiates the subscription handshake, processes JSON frames on the fly, standardizes the schema, and securely publishes structured records asynchronously to the Kafka Broker using the high-performance `kafka-python-ng` library.
...
### 4. The Analytics Database: ClickHouse
...
The ingestion workflow uses three crucial layers (defined in [clickhouse-init/init.sql](file:///Users/sysung98/Documents/Projects/crypto-streaming-pipeline/clickhouse-init/init.sql)):
...
2. **The Persistent Table (`crypto_trades_raw`)**: A highly optimized `MergeTree` table that serves as the permanent data vault. It aggregates incoming records on disk and organizes them by `(symbol, trade_time)`.
3. **The Materialized View Pump (`mv_crypto_trades_raw`)**: An active background daemon that binds the Kafka Engine Queue directly to the Persistent Table, performing on-the-fly timestamp conversions as records flow through.
